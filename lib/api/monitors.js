...
module.exports = Class.create({
	...
	api_swap_monitor_props: function(args, callback) {
		// swap specified key of two monitors (a, b, key)
		var self = this;
		var params = args.params;
		
		if (!this.requireParams(params, {
			a: /^\w+$/, // id of first monitor
			b: /^\w+$/, // id of second monitor
		...
		this.loadSession(args, function(err, session, user) {
			...
			var a = Tools.findObject( self.monitors, { id: params.a } );
			if (!a) return self.doError('monitor', "Cannot find monitor with ID: " + params.a, callback);
			
			var b = Tools.findObject( self.monitors, { id: params.b } );
			if (!b) return self.doError('monitor', "Cannot find monitor with ID: " + params.b, callback);
			...
			async.series(
				[
					function(callback) {
						// update monitor A with B's value
						var updates = {};
						updates[ params.key ] = b_value;
						
						self.storage.listFindUpdate( 'global/monitors', { id: params.a }, updates, function(err, monitor) {
					...
					function(callback) {
						// update monitor B with A's value
						var updates = {};
						updates[ params.key ] = a_value;
						
						self.storage.listFindUpdate( 'global/monitors', { id: params.b }, updates, function(err, monitor) {
							if (err) return callback(err);
				...
				function(err) {
					if (err) {
						self.doError('monitor', "Could not update monitors: " + err, callback);
					}
