...
module.exports = Class.create({
	...
	api_config: function(args, callback) {
		...
		async.parallel(
			[
				function(callback) {
					// get first entry in contrib/daily to extract the oldest year we have data for
					// this is for the UI year menu range
					self.storage.listGet( 'contrib/daily', 0, 1, function(err, items, list) {
						if (items && items.length && items[0].date) {
							var dargs = Tools.getDateArgs(items[0].date);
							resp.first_year = dargs.year;
						}
						else {
				...
				function(callback) {
					...
					var dargs = Tools.getDateArgs( new Date() );
					self.storage.get( 'contrib/daily/' + dargs.yyyy_mm_dd, function(err, data) {
						var hostnames = {};
