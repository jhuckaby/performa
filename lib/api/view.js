...
module.exports = Class.create({
	...
	api_contrib: function(args, callback) {
		...
		if (query.date) {
			...
			this.storage.get( contrib_key, function(err, data) {
			...
			// malformed request
			return this.doError('contrib', "Missing both date and length properties", callback);
		}
	},
	...
	api_view: function(args, callback) {
		...
		if (query.date) {
			...
			this.storage.listGet( timeline_key, 0, 0, function(err, items) {
	...
	api_overview: function(args, callback) {
		...
		this.storage.listGet( timeline_key, offset, length, function(err, items) {
			if (err || !items || !items.length) return self.doError('no_data', "No data found", callback);
